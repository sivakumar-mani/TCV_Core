...
export class InternetCustomerList {
  ...
  colDefs:ColDef[]=[
    ...
    {field:'display_customer_no',headerName:'Cust No',width:88,minWidth:88,maxWidth:88,flex:0},
  ...
  filter(){const text=(value:any)=>String(value||'').trim().toLowerCase();this.rows=this.all.filter(x=>(!this.filters.customerNo||text(x.display_customer_no).includes(text(this.filters.customerNo)))&&(!this.filters.network||text(x.network_type)===text(this.filters.network))&&(!this.filters.name||text(x.full_name).includes(text(this.filters.name)))&&(!this.filters.mobile||text(x.mobile_no).includes(text(this.filters.mobile)))&&(!this.filters.status||text(x.status)===text(this.filters.status)));}
