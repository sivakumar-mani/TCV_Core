...
export class Sidebar {
  ...
  readonly groups = [
    ...
    { label: 'Accounts', icon: 'bi-bank', items: [['Transactions', '/transactions', 'TRANSACTIONS'], ['Pending Accounts', '/cable-tv-account-pending', 'CABLE_TV_ACCOUNTS'], ['LO Accounts', '/lo-accounts', 'CABLE_TV_ACCOUNTS'], ['CATV Subscription', '/cable-tv-subscription-pending', 'CABLE_TV_SUBSCRIPTION_DUES'], ['Net Subscription', '/net-subscription-pending', 'NET_SUBSCRIPTION'], ['Append CATV Subscriptions', '/cable-tv-subscription-append', 'CABLE_TV_SUBSCRIPTION_GENERATE'], ['Append Net Subscriptions', '/net-subscription-append', 'NET_SUBSCRIPTION_APPEND']] },
    { label: 'Reports', icon: 'bi-bar-chart', items: [['CATV Subscription Report', '/cable-tv-subscription-report', 'CABLE_TV_SUBSCRIPTION_REPORT'], ['STB Payment Report', '/stb-payment-report', 'CABLE_TV_SUBSCRIPTION_REPORT'], ['Net Subscription Report', '/net-subscription-report', 'INTERNET_CUSTOMERS']] },
  ...
  itemVisible(item: string[]) { return item[3] !== 'ADMIN' || this.permissions.isAdmin() ? this.permissions.can(item[2]) : false; }
  itemIcon(route: string) {
    const icons: Record<string, string> = {
      ...
      '/cable-tv-account-pending': 'bi-hourglass-split', '/lo-accounts': 'bi-broadcast-pin', '/cable-tv-subscription-pending': 'bi-tv-fill',
