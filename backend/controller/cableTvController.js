...
const ensureCableTvExtendedTables = async (db) => {
  ...
  if (!customerTypeColumn.count) {
    await db.query("ALTER TABLE cable_tv_customers ADD COLUMN customer_type ENUM('REGULAR','BUSINESS') NOT NULL DEFAULT 'REGULAR' AFTER network_type");
  }
  ...
  await db.query(
    `UPDATE cable_customer_accounts
     SET stb_amount = ?, connection_amount = ?, labor_amount = ?, material_cost = ?,
         ...
         balance_amount = ?, office_balance_amount = ?, updated_at = NOW()
     WHERE account_id = ?`,
    [
      ...
      Math.max(grandTotal - officeReceived, 0), account.account_id
    ]
  );
};
...
const addStbMaster = async (req, res) => {
  try {
    ...
    const canReAddAsServiced = duplicates.length > 0
      && stockType === 'SERVICED'
      && duplicates.every((duplicate) => duplicate.stock_type === 'FAULT');
    if (duplicates.length > 0 && !canReAddAsServiced) {
      return res.status(409).json({
        message: 'STB number already exists. A Fault, Damaged or Burnt STB can be added again only as Serviced.'
      });
    }
...
const updateStbMaster = async (req, res) => {
  try {
    ...
    const canUseFaultedNumberAsServiced = duplicates.length > 0
      && stockType === 'SERVICED'
      && duplicates.every((duplicate) => duplicate.stock_type === 'FAULT');
    if (duplicates.length > 0 && !canUseFaultedNumberAsServiced) {
      return res.status(409).json({
        message: 'STB number already exists. A Fault, Damaged or Burnt STB can be used again only as Serviced.'
      });
    }
  ...
  try {
    ...
    const [materialRows] = await db.query(
      `SELECT -m.material_movement_id AS account_id, NULL AS approval_group_id, m.cable_customer_id,
              ...
              NULL AS received_by_name, 0 AS stb_amount, 0 AS connection_amount, 0 AS labor_amount,
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT c.cable_customer_id, c.customer_code, c.legacy_customer_no, c.full_name,
               ...
               CASE
                 WHEN UPPER(c.approval_status) = 'PENDING' THEN 'Waiting Approval'
                 WHEN UPPER(c.approval_status) = 'REJECTED' THEN 'Rejected'
                 WHEN UPPER(COALESCE(acc.account_status, 'PENDING')) IN ('PENDING', 'PARTIAL')
                   AND NOT EXISTS (
...
const addCableCustomer = async (req, res) => {
  ...
  try {
    ...
    if (!['REGULAR', 'BUSINESS'].includes(customerType)) {
      await db.rollback();
      return res.status(400).json({ message: 'Customer Type must be Regular or Business' });
    }
    const [customerResult] = await db.query(
      `INSERT INTO cable_tv_customers (
        approval_group_id, network_id, network_type, customer_type, legacy_customer_no, customer_code, full_name, door_no,
      ...
      [
        approvalGroupId, networkId, networkType, customerType, nullable(payload.legacy_customer_no), customerCode, payload.full_name, payload.door_no,
  ...
  try {
    ...
    await db.query(
      `UPDATE cable_tv_customers SET
        network_id = ?, network_type = ?, customer_type = ?, legacy_customer_no = ?, full_name = ?, door_no = ?, location_id = ?,
      ...
      [
        networkId, networkType, customerType, nullable(payload.legacy_customer_no), payload.full_name, payload.door_no,
...
const addCustomerConnection = async (req, res) => {
  ...
  try {
    ...
    if (approvalStatus === 'APPROVED' && connectionType === 'SHIFTED') {
      await db.query(
        `UPDATE cable_tv_customers
         SET door_no = ?, location_id = ?, area_id = ?, street_id = ?, city = ?, pincode = ?, updated_at = NOW()
         WHERE cable_customer_id = ?`,
  ...
  try {
    ...
    const [[pendingStbUpdate]] = await db.query(
      `SELECT customer_stb_id
       ...
       WHERE cable_customer_id = ? AND approval_status = 'PENDING'
       LIMIT 1`,
      [cableCustomerId]
    );
    if (pendingStbUpdate) {
    ...
    const [activeStbs] = await db.query(
      `SELECT customer_stb_id, stb_master_id, stb_type, installed_mso_id, exchange_original_mso_id,
              stb_no, stb_image_path, installed_by_employee_id, installed_date, status
       FROM cable_customer_stbs
    ...
    if (currentStatus && currentStatus !== 'ACTIVE' && !disconnectedReasons.has(updateReason)) {
      await db.rollback();
      return res.status(400).json({ message: 'Disconnected STB can only be Reactivated or Replaced' });
    }
    ...
    if (!isReplacement) {
      ...
      const [statusResult] = await db.query(
        ...
        [
          ...
          faultReasons.has(updateReason) ? 'FAULT' : updateReason === 'RETURNED' ? 'RETURNED' : targetStb.stb_type || 'NEW',
      ...
      if (approvalStatus === 'APPROVED') {
        ...
        if (isReturn && targetStb.stb_master_id) {
          await db.query(
            "UPDATE cable_stb_master SET stock_type = 'RETURNED', status = 'AVAILABLE', assigned_employee_id = NULL, updated_at = NOW() WHERE stb_master_id = ?",
            [targetStb.stb_master_id]
          );
          await db.query(
...
const updateCustomerStb = async (req, res) => {
  try {
    ...
    const [[existingStb]] = await db.query(
      `SELECT customer_stb_id, approval_group_id, stb_master_id, stb_type, stb_no, update_reason
       FROM cable_customer_stbs
...
const updateCustomerSubscription = async (req, res) => {
  try {
    ...
    if (!isAdmin(req)) {
      if (String(existingSubscription.customer_status || '').toUpperCase() !== 'ACTIVE') {
        return res.status(403).json({ message: 'Subscription payment can be updated only for an active customer' });
      }
      ...
      const [updateResult] = await db.query(
        ...
        [
          ...
          dateOnly(existingSubscription.collect_date || existingSubscription.created_at),
          employeeId, subscriptionId, cableCustomerId
        ]
      );
    ...
    const paymentStatus = ['PENDING', 'PARTIAL', 'PAID'].includes(requestedStatus) ? requestedStatus : 'PENDING';
