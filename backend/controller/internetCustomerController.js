...
// Online renewals retain the existing CUSTOMER database value.
const subscriptionRenewal = (admin, payload) => {
  if (!admin) return { renewedBy: 'ADMIN', paymentMode: 'CASH' };
  const renewedBy = String(payload.renewed_by_value || '').toUpperCase();
  const paymentMode = String(payload.payment_mode || '').toUpperCase();
  if (!['ADMIN', 'CUSTOMER'].includes(renewedBy) ||
      !(renewedBy === 'CUSTOMER' ? paymentMode === 'DASHBOARD' : ['CASH', 'ACCOUNT'].includes(paymentMode))) {
    throw Object.assign(new Error('Select Online with Dashboard, or Admin with Cash or Account'), { status: 400 });
  }
  return { renewedBy, paymentMode };
};
const correctionSelection = (payload) => {
  const selection = subscriptionRenewal(true, payload);
  const status = String(payload.payment_status || '').toUpperCase().replace('UNPAID', 'PENDING');
  if (!['PAID', 'PENDING'].includes(status)) throw Object.assign(new Error('Select Paid or Unpaid'), { status: 400 });
...
const initializeInternetSchema = async (db) => {
  ...
  await db.query("ALTER TABLE internet_subscriptions MODIFY payment_mode ENUM('DASHBOARD','CASH','ONLINE','BANK_PAYMENT','ACCOUNT') NOT NULL DEFAULT 'DASHBOARD'");
...
const getInternetCustomers = async (_req, res) => {
  try {
    const db = connection.promise(); await ensureInternetSchema(db);
    const [rows] = await db.query(`SELECT c.*, ${internetCustomerNumberSql('c')} display_customer_no, l.location_name, a.area_name, s.street_name,
...
const getInternetCustomer = async (req, res) => {
  try {
    ...
    const [[customer]] = await db.query(`SELECT c.*, ${internetCustomerNumberSql('c')} display_customer_no, l.location_name, a.area_name, s.street_name,
...
const addInternetCustomerHistory = async (req,res) => {
  ...
  try{
    ...
    // Subscription renewals can be recorded while an earlier account payment is pending.
...
const getPendingInternetSubscriptions = async (req,res) => {
  try {
    ...
    if(req.query.customer_no){conditions.push(`${internetCustomerNumberSql('c')} LIKE ?`);values.push(`%${String(req.query.customer_no).trim()}%`);}
    if(String(req.query.net_id||'').trim()){conditions.push('LOWER(c.net_id) LIKE ?');values.push(`%${String(req.query.net_id).trim().toLowerCase()}%`);}
    ...
    const [rows]=await db.query(`SELECT s.*,c.internet_customer_id,c.customer_code,c.legacy_customer_no,${internetCustomerNumberSql('c')} display_customer_no,c.full_name,c.net_id,c.network_type,c.door_no,c.city,c.pincode,c.installed_date,c.status customer_status,a.area_name,st.street_name,pm.package_name,cp.package_price FROM internet_subscriptions s JOIN internet_customers c ON c.internet_customer_id=s.internet_customer_id LEFT JOIN cable_areas a ON a.area_id=c.area_id LEFT JOIN cable_streets st ON st.street_id=c.street_id LEFT JOIN internet_customer_packages cp ON cp.internet_customer_package_id=s.internet_customer_package_id LEFT JOIN internet_package_master pm ON pm.package_id=cp.package_id WHERE ${conditions.join(' AND ')} ORDER BY CAST(${internetCustomerNumberSql('c')} AS UNSIGNED),s.start_date`,values);
...
const getUnassignedNetCollectors = async (req,res) => {
  try {
    const db=connection.promise();await ensureInternetSchema(db);
    const conditions=["s.payment_status='PAID'",'s.balance_amount=0',"s.approval_status<>'REJECTED'","c.approval_status='APPROVED'",'(s.collected_by_employee_id IS NULL OR s.collect_date IS NULL)'],values=[];
    for(const [key,column] of [['customer_no',internetCustomerNumberSql('c')],['net_id','c.net_id'],['customer_name','c.full_name']]) {
      const value=String(req.query[key]||'').trim().toLowerCase();
      if(value){conditions.push(`LOWER(${column}) LIKE ?`);values.push(`%${value}%`);}
    }
    for(const key of ['area_id','street_id'])if(req.query[key]){conditions.push(`c.${key}=?`);values.push(Number(req.query[key]));}
    const [customers]=await db.query(`SELECT s.*,${internetCustomerNumberSql('c')} display_customer_no,c.full_name,c.net_id,c.network_type,c.door_no,c.city,c.pincode,c.installed_date,c.status customer_status,a.area_name,st.street_name,CONCAT_WS(' ',e.first_name,e.last_name) collected_by_name FROM internet_subscriptions s JOIN internet_customers c ON c.internet_customer_id=s.internet_customer_id LEFT JOIN cable_areas a ON a.area_id=c.area_id LEFT JOIN cable_streets st ON st.street_id=c.street_id LEFT JOIN employees e ON e.employee_id=s.collected_by_employee_id WHERE ${conditions.join(' AND ')} ORDER BY s.subscription_year DESC,s.subscription_month DESC,c.net_id`,values);
    return res.json({customers});
  }catch(error){return res.status(500).json({message:error.message});}
};
const assignNetCollector = async (req,res) => {
  const db=connection.promise();
  try {
    await ensureInternetSchema(db);await db.beginTransaction();
    const [[row]]=await db.query(`SELECT s.* FROM internet_subscriptions s JOIN internet_customers c ON c.internet_customer_id=s.internet_customer_id WHERE s.internet_subscription_id=? AND s.payment_status='PAID' AND s.balance_amount=0 AND s.approval_status<>'REJECTED' AND c.approval_status='APPROVED' AND (s.collected_by_employee_id IS NULL OR s.collect_date IS NULL) FOR UPDATE`,[Number(req.params.id)]);
    if(!row)throw Object.assign(new Error('Subscription is no longer awaiting collector assignment'),{status:409});
    const p=req.body,employee=await resolveLoggedInEmployeeId(db,req),collector=isAdmin(req)?intOrNull(p.collected_by_employee_id):employee;
    const [[validEmployee]]=await db.query('SELECT employee_id FROM employees WHERE employee_id=? AND is_active=1',[collector]);
    if(!validEmployee)throw Object.assign(new Error('Select an active collector mapped to an employee'),{status:400});
    const start=dateOnly(p.start_date),end=dateOnly(p.end_date),collectDate=isAdmin(req)?dateOnly(p.collect_date):dateOnly(new Date());
    const value=Number(p.period_value),count=Number(p.period_count),free=Number(p.free_period_value),paid=Number(p.paid_amount),units=['MONTH','DAYS','YEAR'];
    const amount=money(Number(row.amount)*count/(Number(row.period_count)||1));
    if(!start||!end||end<start||!collectDate||!Number.isInteger(value)||value<1||!Number.isFinite(count)||count<=0||!Number.isInteger(free)||free<0||!units.includes(p.period_unit)||!units.includes(p.free_period_unit)||!Number.isFinite(paid)||paid<0||paid>amount)throw Object.assign(new Error('Enter valid periods, dates and paid amount'),{status:400});
    const balance=money(amount-paid),status=balance===0?'PAID':paid>0?'PARTIAL':'PENDING';
    await db.query(`UPDATE internet_subscriptions SET billing_basis=?,period_value=?,period_count=?,free_period_value=?,free_period_unit=?,start_date=?,end_date=?,amount=?,paid_amount=?,balance_amount=?,payment_status=?,collect_date=?,collected_by_employee_id=?,payment_mapped_employee_id=?,payment_reference=? WHERE internet_subscription_id=?`,[p.period_unit,value,count,free,p.free_period_unit,start,end,amount,paid,balance,status,collectDate,collector,collector,textOrNull(p.payment_reference),row.internet_subscription_id]);
    await syncNetEnrollmentAccount(db,row.initial_account_id);await db.commit();return res.json({message:'Net collector assigned successfully'});
  }catch(error){await db.rollback();return res.status(error.status||500).json({message:error.message});}
};
...
const receiveInternetSubscriptionPayment = async (req,res) => {
  const db=connection.promise();try{await ensureInternetSchema(db);await db.beginTransaction();const id=Number(req.params.id);const [[row]]=await db.query(`SELECT s.* FROM internet_subscriptions s JOIN internet_customers c ON c.internet_customer_id=s.internet_customer_id WHERE s.internet_subscription_id=? AND s.approval_status<>'REJECTED' AND c.approval_status='APPROVED' FOR UPDATE`,[id]);if(!row)throw Object.assign(new Error('Internet subscription not found'),{status:404});
    ...
    const paid=money(Number(row.paid_amount)+received),remaining=money(Math.max(amount-paid,0)),status=remaining===0?'PAID':'PARTIAL';const collectedBy=isAdmin(req)?intOrNull(req.body.collected_by_employee_id)||employeeId:employeeId;const collectDate=isAdmin(req)?dateOnly(req.body.collect_date||new Date()):dateOnly(new Date());
    const {renewedBy,paymentMode:mode}=subscriptionRenewal(isAdmin(req),req.body),renewedEmployee=null;
    if(!employeeId&&!isAdmin(req))throw Object.assign(new Error('Logged-in user is not mapped to an employee'),{status:400});
...
const updateInternetSubscription = async (req,res) => {
  ...
  try {
    ...
    const {renewedBy,paymentMode}=subscriptionRenewal(isAdmin(req),p),renewedEmployee=null,collectDate=isAdmin(req)?dateOnly(p.collect_date):dateOnly(new Date()),collectedBy=isAdmin(req)?intOrNull(p.collected_by_employee_id):employeeId;
...
const deleteInternetSubscription = async (req,res) => {
  ...
  try{
    if(!isAdmin(req))return res.status(403).json({message:'Administrator permission is required'});
    await ensureInternetSchema(db);await db.beginTransaction();
    const ids=[Number(req.params.subscriptionId),Number(req.params.id)];
    const [[row]]=await db.query('SELECT initial_account_id FROM internet_subscriptions WHERE internet_subscription_id=? AND internet_customer_id=? FOR UPDATE',ids);
    if(!row){await db.rollback();return res.status(404).json({message:'Internet subscription not found'});}
    await db.query('DELETE FROM internet_subscriptions WHERE internet_subscription_id=? AND internet_customer_id=?',ids);
    await syncNetEnrollmentAccount(db,row.initial_account_id,'subscriptions');await db.commit();
    return res.json({message:'Internet subscription deleted successfully'});
  }catch(error){await db.rollback();return res.status(500).json({message:'Internet subscription delete failed',error:error.message});}
};
...
const previewCashAdminCorrection=async(req,res)=>{try{if(!isAdmin(req))return res.status(403).json({message:'Administrator permission is required'});const db=connection.promise();correctionSelection(req.body);await ensureInternetSchema(db);return res.json(await cashAdminCorrectionRows(db,parseNetIds(req.body.net_ids),req.body.subscription_month,req.body.subscription_year));}catch(error){return res.status(error.status||500).json({message:error.message||'Cash/Admin correction preview failed'});}};
const applyCashAdminCorrection=async(req,res)=>{const db=connection.promise();try{if(!isAdmin(req))return res.status(403).json({message:'Administrator permission is required'});const selection=correctionSelection(req.body);await ensureInternetSchema(db);await db.beginTransaction();const preview=await cashAdminCorrectionRows(db,parseNetIds(req.body.net_ids),req.body.subscription_month,req.body.subscription_year),customerIds=preview.customers.filter(row=>Number(row.subscription_count)>0).map(row=>Number(row.internet_customer_id));if(!customerIds.length)throw Object.assign(new Error(`No subscriptions were found for ${preview.month}/${preview.year} and the supplied Net IDs`),{status:400});const placeholders=customerIds.map(()=>'?').join(',');const [result]=await db.query(`UPDATE internet_subscriptions SET payment_mode=?,renewed_by=?,renewed_by_employee_id=NULL,cash_admin_locked=0,payment_status=?,paid_amount=CASE WHEN ?='PAID' THEN amount ELSE 0 END,balance_amount=CASE WHEN ?='PAID' THEN 0 ELSE amount END WHERE internet_customer_id IN (${placeholders}) AND subscription_month=? AND subscription_year=? AND approval_status<>'REJECTED'`,[selection.paymentMode,selection.renewedBy,selection.status,selection.status,selection.status,...customerIds,preview.month,preview.year]);await db.commit();return res.json({message:`${result.affectedRows} subscription(s) updated successfully for ${preview.month}/${preview.year}`,updated_subscriptions:result.affectedRows,matched_customers:customerIds.length,month:preview.month,year:preview.year,unmatched_net_ids:preview.unmatched_net_ids});}catch(error){try{await db.rollback();}catch(_e){}return res.status(error.status||500).json({message:error.message||'Cash/Admin correction failed'});}};
...
module.exports={getUnassignedNetCollectors,assignNetCollector,ensureInternetSchema,internetLookups,getInternetCustomers,getInternetCustomer,saveInternetCustomer,getInternetComplaints,addInternetComplaint,updateInternetCustomerInformation,addInternetCustomerHistory,getPendingInternetSubscriptions,receiveInternetSubscriptionPayment,updateInternetSubscription,deleteInternetSubscription,updateInternetCustomerPackage,deleteInternetCustomerPackage,updateInternetCustomerRouter,deleteInternetCustomerRouter,previewInternetSubscriptionAppend,appendInternetSubscriptions,previewCashAdminCorrection,applyCashAdminCorrection,getInternetSubscriptionReport};
