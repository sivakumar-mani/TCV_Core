...
const addInternetCustomerHistory = async (req,res) => {
  ...
  try{
    ...
    const [[latest]]=await db.query('SELECT account_status FROM internet_customer_accounts WHERE internet_customer_id=? ORDER BY internet_account_id DESC LIMIT 1',[customerId]);
    ...
    const approval=isAdmin(req)?'APPROVED':'PENDING';let routerAmount=0,connectionAmount=0,laborAmount=0,subscriptionAmount=0;
    if(section==='routers'){
      const routerType=['NEW','SERVICED','RETURNED'].includes(String(p.router_type||'').toUpperCase())?String(p.router_type).toUpperCase():'NEW',qty=Math.max(money(p.qty||1),1);
      const [[product]]=routerType==='RETURNED'
      ...
      routerAmount=amount;const [routerResult]=await db.query('INSERT INTO internet_customer_routers(internet_customer_id,router_type,usage_category,product_id,hsn_code,qty,unit,rate,discount,amount,remarks,returned_router_qty,returned_adapter_product_id,returned_adapter_qty,refund_amount,refund_payment_mode,refund_status,updated_by_employee_id,stock_processed,approval_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[customerId,routerType,category,product.product_id,product.hsn_code,qty,product.unit||'PCS',rate,discount,amount,textOrNull(p.remarks),returnedRouterQty,adapterProductId,returnedAdapterQty,refundAmount,refundMode,refundAmount>0?'PENDING':'NOT_APPLICABLE',employeeId,isAdmin(req)?1:0,approval]);
      if(isAdmin(req)){
        const movements=routerType==='RETURNED'?[[product.product_id,returnedRouterQty],[adapterProductId,returnedAdapterQty]]:[[product.product_id,-qty]];
      ...
      connectionAmount=money(p.connection_charge);laborAmount=money(p.labour_service_charge);
      const [[activePackage]]=await db.query('SELECT package_price FROM internet_customer_packages WHERE internet_customer_id=? AND is_active=1 AND approval_status=\'APPROVED\' ORDER BY internet_customer_package_id DESC LIMIT 1',[customerId]);
      subscriptionAmount=money(activePackage?.package_price);
      await db.query('INSERT INTO internet_connections(internet_customer_id,connection_date,connection_type,connection_charge,connection_discount,labour_service_charge,remarks,approval_status) VALUES(?,?,?,?,?,?,?,?)',[customerId,dateOnly(p.connection_date),p.connection_type||'RECONNECTION',connectionAmount,money(p.connection_discount),laborAmount,textOrNull(p.remarks),approval]);
    ...
    const grand=money(routerAmount+connectionAmount+subscriptionAmount),paid=money(p.customer_paid_amount),balance=Math.max(grand-paid,0);
    if(!['subscriptions','packages'].includes(section))await db.query('INSERT INTO internet_customer_accounts(internet_customer_id,account_source,router_amount,connection_amount,labor_amount,subscription_amount,grand_total,customer_paid_amount,office_received_amount,office_balance_amount,balance_amount,account_status,approval_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[customerId,section.slice(0,-1).toUpperCase(),routerAmount,connectionAmount,0,subscriptionAmount,grand,paid,0,grand,balance,'PENDING',approval]);
    if(!isAdmin(req))await db.query(`INSERT INTO workflow_approvals(module_name,reference_id,reference_no,workflow_status,requested_by_employee_id,remarks) VALUES('INTERNET_CUSTOMER_UPDATE',?,?,'PENDING',?,'Internet customer details update') ON DUPLICATE KEY UPDATE workflow_status='PENDING',requested_by_employee_id=VALUES(requested_by_employee_id),reviewed_at=NULL,remarks=VALUES(remarks)`,[customerId,String(customer.customer_code),employeeId]);
