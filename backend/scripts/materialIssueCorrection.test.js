...
function setup({ role = 'ADMIN', method = 'PATCH', stock = 10, payments = [], adjustments = [], status = 'ISSUED' } = {}) {
  const calls = [];
  const db = {
  ...
  const sandbox = { require: name => { assert.equal(name, '../connection'); return { promise: () => db }; }, module: { exports: {} } };
...
test('dependent records, sold entries and insufficient stock roll back', async () => {
  for (const options of [{ payments: [{}] }, { adjustments: [{}] }, { status: 'SOLD' }, { stock: 1 }]) {
