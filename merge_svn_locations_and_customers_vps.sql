...
-- APPLY MODE: 1 saves the validated changes; use 0 only for a read-only preview.
-- Requires CREATE ROUTINE permission. No customer, street or area is deleted.
SET @apply_svn_merge = 1;
SET @svn_merge_result = 'NOT COMPLETED';
-- No result sets are emitted inside CALL (phpMyAdmin compatibility).
DROP PROCEDURE IF EXISTS tcv_merge_svn_locations_20260926;
DELIMITER $$
CREATE PROCEDURE tcv_merge_svn_locations_20260926(IN do_apply INT)
main: BEGIN
 DECLARE net INT;
 DECLARE chrom INT;
 DECLARE pam INT;
 ...
 DECLARE step_no INT DEFAULT 1;
 DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN ROLLBACK; SET @svn_merge_result='ROLLED BACK: no merge committed'; RESIGNAL; END;

 START TRANSACTION;
 SELECT COUNT(*),MIN(network_id) INTO found_count,net FROM cable_network_master
 WHERE UPPER(TRIM(network_code))='SVN' AND is_active=1;
 IF found_count<>1 THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Expected one active SVN network'; END IF;
 ...
 WHILE step_no<=2 DO
   SET target_name=IF(step_no=1,'SHANKAR NAGAR','PRASANTHI NAGAR');
   SET source_post=IF(step_no=1,chrom,pam);
   SET target_post=IF(step_no=1,pam,chrom);
   -- Resolve the two postal locations separately: one source and one destination
   -- with the same name is an expected merge, not an ambiguity.
   SELECT COUNT(*),MIN(area_id) INTO found_count,source_area FROM cable_areas
   WHERE network_id=net AND location_id=source_post AND UPPER(TRIM(area_name))=target_name AND is_active=1;
   IF found_count>1 THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Multiple active source areas in same postal area; exact IDs need review'; END IF;
   ...
   IF source_area IS NULL AND target_area IS NULL THEN
     SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Requested SVN area name not found; check spelling';
   END IF;
   ...
   THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Other-network customers reference SVN areas; review first'; END IF;
 ...
 IF do_apply=1 THEN
   COMMIT;
   SET @svn_merge_result = 'COMMITTED: SVN Location Info, CATV and Internet customer addresses updated';
 ELSE
   ROLLBACK;
   SET @svn_merge_result = 'PREVIEW ONLY: review IDs, set @apply_svn_merge=1 and rerun WHOLE file';
 END IF;
END$$
DELIMITER ;
CALL tcv_merge_svn_locations_20260926(@apply_svn_merge);
DROP PROCEDURE tcv_merge_svn_locations_20260926;

-- Ordinary result queries outside CALL avoid nested/multiple procedure results.
SELECT @svn_merge_result AS result;
SELECT n.network_code,l.location_name AS postal_area,a.area_id,a.area_name,a.is_active,
       CASE WHEN UPPER(TRIM(a.area_name))='SHANKAR NAGAR' THEN 'PAMMAL' ELSE 'CHROMEPET' END AS intended_postal_area,
...
WHERE UPPER(TRIM(n.network_code))='SVN' AND UPPER(TRIM(a.area_name)) IN ('SHANKAR NAGAR','PRASANTHI NAGAR')
UNION ALL
